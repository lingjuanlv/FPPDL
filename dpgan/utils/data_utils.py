...
# from tf_DataSet import read_data_sets
...
class MNISTLoader(DataLoader):
    ...
    def __init__(self, data_dir="/tmp/mnist", normalize=True, one_hot=True,
                 include_train=True, include_test=True,
                 first=None, last=None, seed=None,train_label_indices=[]):
        # mnist = read_data_sets(train_dir=data_dir)
        mnist = tf_DataSet.read_data_sets(train_dir=data_dir,validation_size=0,shard_index=train_label_indices[0:len(train_label_indices)])
        data, labels = [], []
        train = mnist.train
        test = mnist.test
...
class MNISTLoader_aug(DataLoader):
    filenames = ["data_batch_1", "data_batch_2", "data_batch_3", "data_batch_4", "data_batch_5", "test_batch"]

    def __init__(self, expanded_images, expanded_labels, normalize=True, one_hot=True,
                 first=None, last=None, seed=None):
        # mnist = read_data_sets(train_dir=data_dir)
        # mnist = tf_DataSet.read_data_sets(train_dir=data_dir,validation_size=0,shard_index=train_label_indices[0:len(train_label_indices)])
        # data, labels = [], []
        ...
        # data = np.concatenate(data, axis=0).reshape((-1, 28, 28, 1)).astype(np.float32)
        # labels = np.concatenate(labels, axis=0)[:, None].astype(np.int64)
        print(len(expanded_images))
        assert first is None or last is None
        if first is not None:
            n = min(first, len(expanded_images))
            indices = np.random.RandomState(seed).permutation(np.arange(0, len(expanded_images), dtype=np.int64))[:n]
            data = expanded_images[indices]
            labels = expanded_labels[indices]
        if last is not None:
            n = min(last, len(expanded_images))
            indices = np.random.RandomState(seed).permutation(np.arange(0, len(expanded_images), dtype=np.int64))[-n:]
            data = expanded_images[indices]
            labels = expanded_labels[indices]
        # print(data[1])
