...
def gradient_norms_estimate_tower(config, discriminator_forward, real_data, fake_data,
    ...
    # return real_data, disc_final_norms, config.batch_size, config.batch_size * config.num_cpu
...
def aggregate_flow(config, disc_costs, gen_costs, disc_grads, gen_optimizer,
    ...
    # final_disc_cost = tf.add_n(disc_costs) / config.num_cpu
    # final_gen_cost = tf.add_n(gen_costs) / config.num_cpu
